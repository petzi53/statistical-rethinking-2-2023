...
The only difference between a good or bad choice is the time (the
different steps) the updating process needs to produce the final result.
...
The `value` column is of type `double` as can be seen in the result of Annotation (3). For the plot it has to be changed to the type of `character`. Otherwise it could not be used with the `fill` option.
...
::: callout-note
###### Using flextable() and labeling the chunk requires table capture as well

If you label the chunk using `flextable()` then you have also to add
`tbl-cap` to the chunk options.
:::

```{r}
#| label: basic-prob-struct
