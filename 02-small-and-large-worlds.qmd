...
:::::{.my-exercise}
:::{.my-exercise-header}
Exercise 2E1: Which of the expressions below correspond to the statement: *the probability of rain on Monday*? {#chap-02-e1}
:::
...
:::::{.my-exercise}
:::{.my-exercise-header}
Exercise 2E3: Which of the expressions below correspond to the statement: *the probability that it is Monday, given that it is raining*? {#chap-02-e3}
:::
::::{.my-exercise-container}
...
:::::{.my-exercise}
:::{.my-exercise-header}
Exercise 2H3: Second birth is a singleton infant.
:::
::::{.my-exercise-container}
...
Pr(twins \mid A) = \frac{1}{3} \text{ = first birth} \\
Pr(twins \mid B) = \frac{2}{3} \text{ = first birth} \\
Pr(singleton) = Pr(singleton \mid A) * Pr(twins \mid A) + \\
+ Pr(singleton \mid B) * Pr(twins \mid B) \\
Pr(singleton) = (0.9 * \frac{1}{3}) + (0.8 * \frac{2}{3}) = \frac{0.9}{3} + \frac{1.6}{3} = \frac{2.5}{3} = \frac{5}{6} \text{ = second birth} \\
\\
Pr(A \mid singleton) = \frac{Pr(singleton | A)Pr(twins \mid A)}{Pr(singleton)} \\
Pr(A \mid singleton) = \frac{0.9 * \frac{1}{3}}{\frac{5}{6}} = \frac{\frac{0.9}{3}}{\frac{5}{6}} = \frac{5.4}{15} = 0.36
\end{align*}
$$ {#eq-2h3}
...
:::{.my-r-code-header}
:::::: {#cnj-2H3}
: Probability of {twins, singleton} for species A
::::::
:::
::::{.my-r-code-container}
...
## the likelihood to birth sinlgeton for species A and B
singleton_likelihood_A <-  1 - twins_likelihood_A
singleton_likelihood_B <-  1 - twins_likelihood_B


# likelihood for each species is Pr(twins) * Pr(singleton)
likelihood_A <- twins_likelihood_A * singleton_likelihood_A
likelihood_B <- twins_likelihood_B * singleton_likelihood_B
...
# print result
glue::glue('Posterior probability for singleton
in second birth for species A = {posterior[1]}.')


```

I had it wrong. Instead of multiplying $Pr(singleton \mid species)$ with the previous calculation of  $Pr(twins \mid species)$ I computed the $Pr(species \mid singleton)$ following the pattern of @eq-2h1.
::::
:::::
...
## I STOPPED HERE!! SECOND PASS (2023-11-05) {.unnumbered}
