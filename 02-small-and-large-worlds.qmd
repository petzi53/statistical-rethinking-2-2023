...
For our globe tossing procedure we use instead of counting a mathematical function to calculate the probability of all combinations.
 ...
 > “In this case, once we add our assumptions that (1) every toss is independent of the other tosses and (2) the probability of W is the same on every toss, probability theory provides a unique answer, known as the `r glossary("binomial distribution")`. This is the common “coin tossing” distribution.” ([McElreath, 2020, p. 33](zotero://select/groups/5243560/items/NFUEVASQ)) ([pdf](zotero://open-pdf/groups/5243560/items/CPFRPHX8?page=52&annotation=E5HEDDW4))
...
plausibility, its `r glossary("Prior Probability")`. This is also true when the number of trials is null ($N = 0$), e.g. even in the initial state of information we need a prior. (See @cnj-fig-bayesian-update, where a flat prior was used.)
::::
:::::


When you have a previous estimate, that can become the prior. As a
result, each estimate (`r glossary("posterior probability")`) becomes then the prior for the next step (as you have seen in @cnj-fig-bayesian-update).
