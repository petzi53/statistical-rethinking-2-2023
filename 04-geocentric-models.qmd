...
>
> It's hard to say where any individual person will end up, but you can
> say with great confidence what the collection of positions will be.
> The distances will be distributed in approximately normal, or
> Gaussian, fashion. This is true even though the underlying
> distribution is binomial. It does this because there are so many more
> possible ways to realize a sequence of left-right steps that sums to
> zero. There are slightly fewer ways to realize a sequence that ends up
> one step left or right of zero, and so on, with the number of possible
> sequences declining in the characteristic bell curve of the normal
> distribution.
...
```{r}
#| label: fig-sim-step-experiment-a
#| fig-cap: "Random walks on the soccer field converge to a normal distribution. The more steps are taken, the closer the match between the real empirical distribution of positions and the ideal normal distribution, superimposed in the last plot in the bottom panel."

# to provide reproducibility and to conform with tidyverse version
set.seed(4)
...
rethinking::dens(small, norm.comp = TRUE)
...
## R code 4.20
plot(sample.mu4_a, sample.sigma4_a, cex = 0.8, pch = 21, col = rethinking::col.alpha(rethinking:::rangi2, 0.1))
...
## R code 4.21
rethinking::dens(sample.mu4_a)
rethinking::dens(sample.sigma4_a)
...
## R code 4.22
rethinking::PI(sample.mu4_a)
rethinking::PI(sample.sigma4_a)
```
