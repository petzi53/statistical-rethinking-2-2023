...
6/9: Mean = `r mean(samples_a)` Median = `r median(samples_a)`
3/3: Mean = `r mean(samples_skewed_a)` Median = `r median(samples_skewed_a)`
...
We saved the exact minimum value as `min_loss_b[1]`, which is `r min_loss_b[1]`. Within sampling error, this is the posterior median as depicted by our samples.
...
l2_b %>%   
  ...
  theme(panel.grid = element_blank())
```
Based on quadratic loss $(d−p)^{2}$, the exact minimum value is `r min_loss2_b[1]`. Within sampling error, this is the posterior mean of our samples.
